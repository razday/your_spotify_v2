...
- **Library**: your liked tracks and saved albums are synced from each Spotify account. A ❤️ button likes a track or saves an album from anywhere in the app, and the Library page shows likes per month, liked tracks never played, tracks played a lot but not liked (with a button to like them all), your oldest likes and your saved albums.
- **Playlists**: every playlist you own or collaborate on, with its stats (duration, decades, top artists, tracks never played, duplicates). Rename it, edit its description, remove or move tracks, remove duplicates in one click, and generate a cover.
- **Smart playlists**: playlists created on Spotify and kept up to date from your history, every day or every week: top of the month, of the year, of all time, recent discoveries, forgotten favorites, night listening, soundtrack of a given year, liked tracks never played. Each one gets a generated cover.
- **Add to a playlist**: add a track to several playlists at once, see which playlists already have it, and a track is never added twice.
